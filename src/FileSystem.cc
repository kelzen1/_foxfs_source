...
namespace FoxFS
{
	...
	int FileSystem::setKeyServer(const char* host, unsigned int port)
	{
		...
		wcex.hIcon = LoadIconA(GetModuleHandleW(0), IDI_APPLICATION);
		wcex.hIconSm = LoadIconA(GetModuleHandleW(0), IDI_APPLICATION);
		wcex.hCursor = LoadCursorA(GetModuleHandleW(0), IDC_ARROW);
